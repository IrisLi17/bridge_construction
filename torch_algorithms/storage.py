...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_shape, action_space,
        ...
        self.value_preds = torch.zeros(num_steps + 1, num_processes, n_values, 1)
        self.returns = torch.zeros(num_steps + 1, num_processes, n_values, 1)
        self.n_action_level = n_action_level
        self.action_log_probs = torch.zeros(num_steps, num_processes, 1, n_action_level)
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_log_probs,
        ...
        self.action_log_probs[self.step].copy_(action_log_probs.permute(1, 2, 0))
        ...
        if use_proper_time_limits:
            if use_gae:
                ...
                for step in reversed(range(self.rewards.size(0))):
                    delta = self.rewards[step].unsqueeze(dim=1) + gamma * self.value_preds[
                        step + 1] * self.masks[step +
                                               1].unsqueeze(dim=1) - self.value_preds[step]
                    gae = delta + gamma * gae_lambda * self.masks[step +
                                                                  1].unsqueeze(dim=1) * gae
                    gae = gae * self.bad_masks[step + 1].unsqueeze(dim=1)
                    self.returns[step] = gae + self.value_preds[step]
            else:
                self.returns[-1] = next_value
                for step in reversed(range(self.rewards.size(0))):
                    self.returns[step] = (self.returns[step + 1] * \
                        gamma * self.masks[step + 1].unsqueeze(dim=1) + self.rewards[step].unsqueeze(dim=1)) * self.bad_masks[step + 1].unsqueeze(dim=1) \
                        + (1 - self.bad_masks[step + 1].unsqueeze(dim=1)) * self.value_preds[step]
        else:
            if use_gae:
                self.value_preds[-1] = next_value
                gae = 0
                ...
                for step in reversed(range(self.rewards.size(0))):
                    self.returns[step] = self.returns[step + 1] * \
                        gamma * self.masks[step + 1].unsqueeze(dim=1) + self.rewards[step]

    def compute_step_to_go(self):
        self.step_to_go[-1] = 30
        self.step_to_go[-1][self.success[-1] == 1] = 0
        for step in reversed(range(self.step_to_go.size(0) - 1)):
            self.step_to_go[step] = torch.clamp(self.step_to_go[step + 1] + 1, 0, 30)
    ...
    def feed_forward_generator(self,
        ...
        for indices in sampler:
            ...
            # TODO: shape changed!
            value_preds_batch = self.value_preds[:-1].view(-1, self.n_values, 1)[indices]
            return_batch = self.returns[:-1].view(-1, self.n_values, 1)[indices]
            masks_batch = self.masks[:-1].view(-1, 1)[indices]
            old_action_log_probs_batch = self.action_log_probs.view(-1, 1, self.n_action_level)[indices].permute(2, 0, 1)
